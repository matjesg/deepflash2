...
__all__ = ['show', 'calculate_weights', 'preproc_labels', 'DeformationField', 'BaseDataset', 'RandomTileDataset',
           'TileDataset']
...
def _pdf_postproc(img, width = None, height = None, inter = cv2.INTER_AREA):
    "Prepare probability density function for efficient sampling"
    ...
    return np.cumsum(resized/np.sum(resized))

# Cell
def calculate_weights(clabels=None, instlabels=None, ignore=None, simple=False,
    ...
    if clabels is None:
        instlabels = np.array(instlabels)
...
# Cell
def preproc_labels(clabels=None, instlabels=None, ignore=None, simple=False,
                      n_dims = 2, bws=10, fds=10, bwf=10, fbr=.1):
    """
    Calculates the weights from the given mask (classlabels `clabels` or `instlabels`).
    """
    ...
    _, labels = cv2.connectedComponents(labels.astype('uint8'), connectivity=4)
    pdf = (labels > 0) + (labels == 0) * fbr
    #pdf = _pdf_postproc(pdf)
    ...
    return (np.expand_dims(labels, axis=-1).astype(np.int32), #workaround 1 class
            pdf.astype(np.float32))
...
class BaseDataset(Dataset):
    ...
    def _preproc(self, file):
        ...
        res = preproc_labels(clabels, instlabels, ignore=ign, n_dims=self.c, bws=self.bws, fds=self.fds, bwf=self.bwf, fbr=self.fbr)
...
def _weights_from_labels(self:BaseDataset, instlabels):
    ...
    for c in range(n_classes):
        ...
        for instance in instances:
            #dt2 = ndimage.morphology.distance_transform_edt(instlabels != instance)
            dt = cv2.distanceTransform((instlabels[c] != instance).astype('uint8'), cv2.DIST_L2, cv2.DIST_MASK_PRECISE)
            frgrd_dist += np.exp(-dt ** 2 / (2*self.fds ** 2))
            min2dist = np.minimum(min2dist, dt)
            newMin1 = np.minimum(min1dist, min2dist)
            newMin2 = np.maximum(min1dist, min2dist)
            min1dist = newMin1
...
class RandomTileDataset(BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        # Random center
        center = np.unravel_index(np.argmax(pdf[:] > np.random.random()), pdf.shape)
        X = self.gammaFcn(self.deformationField.apply(img, center).flatten()).reshape((*self.tile_shape, n_channels))
        X = np.moveaxis(X, -1, 0)
        Y = self.deformationField.apply(lbl, center, self.padding, 0)
        Y, W = self._weights_from_labels(Y)
        Y = Y.astype('int64')
...
class TileDataset(BaseDataset):
    ...
    def reconstruct_from_tiles_zarr(self, tiles:zarr.hierarchy.Group, dtype='float32', create_masks=False):
        "Reconstruct masks or predictions from tiles saved in .zarr format"

        root = zarr.group(tiles.chunk_store)
        out = root.create_group(f'{tiles.basename}_reconstr', overwrite=True)
        if create_masks: out_masks = root.create_group(f'{tiles.basename}_reconstr_masks', overwrite=True)

        add = 0
        for idx in range(len(self)):
            outIdx, outShape = self.image_indices[idx], self.image_shapes[idx]
            outSlice, inSlice = self.out_slices[idx], self.in_slices[idx]
            if add < outIdx + 1:
                if create_masks: b = out_masks.zeros(self.files[outIdx].stem, shape=outShape, dtype='uint8', overwrite=True)
                outShape = tuple((*outShape, tiles[idx].shape[-1])) if len(tiles[idx].shape)>2 else outShape
                a = out.zeros(self.files[outIdx].stem, shape=outShape, dtype=dtype, overwrite=True)
                add += 1
            a[outSlice] = tiles[idx][inSlice]
            if create_masks: b[outSlice] = np.argmax(a[outSlice], axis=-1)#
        if create_masks: return out, out_masks
        else: return out