...
__all__ = ['ImgMskTuple', 'calculate_weights', 'DeformationField', 'RandomTileDataset', 'TileDataset']
...
# Cell
class ImgMskTuple(Tuple):
    def show(self, file_name=None, overlay=False, figsize=(10,10), **kwargs):

        if isinstance(self[1], tuple):
            img,msk,weight = self[0], self[1][0], self[1][1]
        elif len(self)>2:
            img,msk,weight = self
        else:
            img,msk = self
...
# Cell
@typedispatch
def show_batch(x:ImgMskTuple, y, samples, max_n=6, ncols=1, figsize=None, **kwargs):
    max_n = np.min((max_n, len(x[0])))
    if figsize is None: figsize = (ncols*12, max_n//ncols * 5)
    for i in range(max_n): ImgMskTuple(x[0][i], x[1][0][i], x[1][1][i]).show(figsize=figsize, **kwargs)
...
class RandomTileDataset(Dataset):
    ...
    def __init__(self,
        ...
        for path in progress_bar(files):
            if path.name not in self.lbl_wgt_pdf:
                ...
                msk = imageio.imread(label_path)//divide
                # Binary mask check
                assert msk.max()== 1, 'Check mask loading'
    ...
    def __getitem__(self, idx):
        ...
        img = imageio.imread(img_path)/self.divide
        assert img.max()> .2 and img.min()<=1., 'Check image loading'
    ...
    def get_images(self, max_n=None):
        ...
        for i in range(max_n):
            path = self.files[i]
            img_list.append(imageio.imread(path)/self.divide)
        return img_list
    ...
    def show_data(self, max_n=6, ncols=1, figsize=None, **kwargs):
        ...
        for i in range(max_n):
            path = self.files[i]
            img = imageio.imread(path)/self.divide
            labels, weights,_ = self.lbl_wgt_pdf[path.name]
            ImgMskTuple(img, labels, weights).show(file_name=path.name)
...
class TileDataset(Dataset):
    ...
    def __init__(self,
        ...
        for i, path in enumerate(progress_bar(files)):
            ...
            labels, weights,_ = self.lbl_wgt_pdf[path.name]
            img = imageio.imread(path)/divide
            assert img.max()> .2 and img.min()<=1., 'Check image loading'
    ...
    def __getitem__(self, idx):
        ...
        Y = np.eye(self.c, dtype='uint8')[self.tile_labels[idx]] if self.label_fn is not None else None
        Y = np.moveaxis(Y, -1, 0)
