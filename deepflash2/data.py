...
__all__ = ['DeformationField', 'DataPreProcessor', 'RandomTileDataset', 'TileDataset']
...
class DataPreProcessor:
    def __init__(
        ...
        foreground_dist_sigma_px=10,#sigma_bal
...
class RandomTileDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        X = np.moveaxis(X, -1, 0).astype('float32')
        Y = self.deformationField.apply(self.labels[idx], center, self.padding, 0)
        # To categorical
        Y = np.eye(self.n_classes, dtype='uint8')[Y]
        Y = np.moveaxis(Y, -1, 0).astype('float32')
