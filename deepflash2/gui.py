...
__all__ = ['query_yes_no', 'ZipUpload', 'ItemsPerPage', 'BaseParamWidget', 'BaseUI', 'PathSelector', 'PathDownloads',
           'PathConfig', 'GTDataSB', 'GTEstSB', 'GTEstUI', 'TrainDataSB', 'TrainModelSB', 'TrainValidSB', 'LRWidget',
           'BasePopUpParamWidget', 'ParamWidget', 'MWWidget', 'TrainUI', 'PredInputSB', 'PredSB', 'PredUI', 'GUI']

# Cell
import sys, shutil, time, zipfile, urllib, subprocess
import ipywidgets as w, numpy as np, pandas as pd
...
_descr = {
    ...
    'up'  : ('Upload Data', 'Upload a zip file. It will be extracted automatically and must contain images or segmentations masks in the correct folder structure'),
...
def _html_wrap(name, tooltip='', url=None):
    ...
    if not url:
        return f'<p title="{tooltip}">{name}</p>'
    else:
        open_tab = 'rel="noopener noreferrer" target="_blank"'#open new tab
        return f'<a href="{url}" title="{tooltip}" {open_tab}>{name}</a>'
...
class GTDataSB:
    ...
    #Grid
    grid = w.GridspecLayout(5, 3, width='100%',  grid_gap="0px", align_items='center')
...
class GTEstSB:
    ...
    grid = w.GridspecLayout(4, 3, width='100%',  grid_gap="0px", align_items='center')
...
class TrainDataSB(BaseParamWidget, GetAttr):
    ...
    grid = w.GridspecLayout(9, 3, width='100%',  grid_gap="0px", align_items='center')
...
class TrainModelSB(BaseParamWidget, GetAttr):
    ...
    #Grid
    grid = w.GridspecLayout(10, 3, width='100%',  grid_gap="0px", align_items='center')
