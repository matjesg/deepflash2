...
__all__ = ['GRID_COLS', 'set_css_in_cell_output', 'tooltip_css', 'query_yes_no', 'ZipUpload', 'ItemsPerPage',
           'ItemsPerPage', 'BaseParamWidget', 'BaseUI', 'PathSelector', 'PathDownloads', 'PathConfig', 'GTDataSB',
           'GTEstSB', 'GTEstUI', 'TrainDataSB', 'TrainModelSB', 'TrainValidSB', 'LRWidget', 'BasePopUpParamWidget',
           'ParamWidget', 'MWWidget', 'TrainUI', 'PredInputSB', 'PredSB', 'PredUI', 'GUI']
...
class ItemsPerPage:
    ...
    up = w.HBox(children=[drp],layout=lyt)
    out = w.Output()
    widget = w.VBox([up, out])

    def __init__(self, plot_fn=None, **kwargs):
    ...
    def on_value_change(self, change):
        ...
        with self.out: self.plot_fn(max_n=change['new'], **self.kwargs)

# Cell
class ItemsPerPage:
    "Dropdown to show n items per page"
    drp = w.Dropdown(options=[5,10,20,50,100], layout=w.Layout(width='auto', min_width='1px'))
    drp_lbl = w.HTML('&emsp;Elements per Page')
    srt = w.Dropdown(options=['alphabetical', 'descending'], layout=w.Layout(width='auto', min_width='1px'))
    srt.layout.display = "none"
    up_left = w.HBox([srt, drp_lbl, drp])

    lyt = w.Layout(width='100px')
    nxt = w.Button(description='Next', layout=lyt)
    prv = w.Button(description='Previous', layout=lyt)
    lbl = w.Label()
    up_right = w.HBox([prv, lbl, nxt])
    ...
    def on_button_clicked(self, b):
        ...
        with self.out:
            self.out.clear_output()
            #print(list(self.items.keys())[slc])
...
class GUI(GetAttr):
    ...
    def gt_data_run_clicked(self, b):
        ...
        with out:
            ...
            out_gt = ItemsPerPage(self.gt_est.show_data, items=items)
            display(out_gt.widget)
    ...
    def pred_data_run_clicked(self, b):
        ...
        with out:
            ...
            out_pred_data = ItemsPerPage(self.el_pred.ds.show_data, items=items, figsize = (5,5))
            display(out_pred_data.widget)
    ...
    def pred_run_clicked(self, b):
        ...
        with out:
            items = {x:x for x in self.el_pred.files}
            out_pred = ItemsPerPage(self.el_pred.show_ensemble_results, items=items)
            display(out_pred.widget)
    ...
    def pred_ood_score_clicked(self, b):
        ...
        with out:
            ...
            #save df_ens!
            self.el_pred.df_ens = self.el_pred.df_ens.sort_values('ood_score', ascending=True).reset_index(drop=True)
            self.el_pred.show_ensemble_results()